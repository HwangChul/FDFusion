...
ABLATION_MODES = ("full", "wo_teb", "wo_ceb", "wo_clma")


def normalize_ablation_mode(mode):
    """Convert user-friendly ablation names to the internal mode names."""
    ...
    aliases = {
        ...
        "without_ceb": "wo_ceb",
        "w/o_clma": "wo_clma",
        "without_clma": "wo_clma",
    }
    normalized = aliases.get(normalized, normalized)
    if normalized not in ABLATION_MODES:
        raise ValueError(
            f"Unknown ablation mode '{mode}'. Expected one of: {', '.join(ABLATION_MODES)}"
        )
    return normalized
...
class JCR(nn.Module):
    def __init__(
        ...
        self.ablation = normalize_ablation_mode(ablation)
        ...
        # Retained for compatibility with checkpoints of the original model.
        ...
        if self.ablation == 'wo_teb':
            # Replace the original TEB/CEB pair with two independent CEBs.
            self.cebA1 = DRAMiTransformer(dim=self.conv_dim, num_head=8, chsa_head_ratio=0.5)
            self.cebA2 = DRAMiTransformer(dim=self.res_dim, num_head=8, chsa_head_ratio=0.5)
            self.cebB1 = DRAMiTransformer(dim=self.conv_dim, num_head=8, chsa_head_ratio=0.5)
            self.cebB2 = DRAMiTransformer(dim=self.res_dim, num_head=8, chsa_head_ratio=0.5)
        elif self.ablation == 'wo_ceb':
            # Replace the original TEB/CEB pair with two independent TEBs.
            self.tebA1 = DenseBlock(self.conv_dim, self.conv_dim)
            self.tebA2 = DenseBlock(self.res_dim, self.res_dim)
            self.tebB1 = DenseBlock(self.conv_dim, self.conv_dim)
            self.tebB2 = DenseBlock(self.res_dim, self.res_dim)
        else:
            # Full model (also used for the CLMA-only ablation).
            self.denseA = DenseBlock(self.conv_dim, self.conv_dim)
            self.ramitA = DRAMiTransformer(dim=self.res_dim, num_head=8, chsa_head_ratio=0.5)
            self.denseB = DenseBlock(self.conv_dim, self.conv_dim)
            self.ramitB = DRAMiTransformer(dim=self.res_dim, num_head=8, chsa_head_ratio=0.5)
    ...
    def forward(self, imageA, imageB, textA, textB):
        ...
        convA, resA = self._encode_a(convA, resA)
        ...
        convB, resB = self._encode_b(convB, resB)
    ...
    def _encode_a(self, conv_feature, res_feature):
        if self.ablation == 'wo_teb':
            return self.cebA1(conv_feature)[0], self.cebA2(res_feature)[0]
        if self.ablation == 'wo_ceb':
            return self.tebA1(conv_feature), self.tebA2(res_feature)
        return self.denseA(conv_feature), self.ramitA(res_feature)[0]

    def _encode_b(self, conv_feature, res_feature):
        if self.ablation == 'wo_teb':
            return self.cebB1(conv_feature)[0], self.cebB2(res_feature)[0]
        if self.ablation == 'wo_ceb':
            return self.tebB1(conv_feature), self.tebB2(res_feature)
        return self.denseB(conv_feature), self.ramitB(res_feature)[0]
...
class Net(nn.Module):
    def __init__(
        ...
        jcr_ablation = 'full' if self.ablation == 'wo_clma' else self.ablation
        self.jcr1 = JCR(ablation=jcr_ablation)
        self.jcr2 = JCR(input_channel=mid_channel, ablation=jcr_ablation)
        self.jcr3 = JCR(input_channel=mid_channel, ablation=jcr_ablation)

        self.clma = None if self.ablation == 'wo_clma' else CLMA()
