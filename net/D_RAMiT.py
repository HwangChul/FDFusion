...
# rearrange时不能被wh、ww整除，进行填充，最后变回原尺寸
def window_partition_pad(x, window_size):
    B, L, C, H, W = x.shape
    ...
    if pad_h > 0 or pad_w > 0:
        ...
        x = F.pad(x, (0, pad_w, 0, pad_h))  # 只pad宽高
    return x, (H, W), (pad_h, pad_w)

def window_unpartition_pad(x, original_hw, window_size, pad_hw):
...
class SpatialSelfAttention(nn.Module):
    ...
    def forward(self, qkv, ch=None):
        ...
        # pad并partition
        qkv_pad, (H_orig, W_orig), (pad_h, pad_w) = window_partition_pad(qkv, self.window_size)
        B, L, C, H_pad, W_pad = qkv_pad.shape
        q, k, v = rearrange(
            qkv_pad,
        ...
        # window_unpartition
        x = window_unpartition(x, (H_pad, W_pad), self.window_size)  # [B, L1*C/L, H_pad, W_pad]

        # 去掉pad回到原尺寸
        x = window_unpartition_pad(x, (H_orig, W_orig), self.window_size, (pad_h, pad_w))
...
class DRAMiT(nn.Module): # Reciprocal Attention Transformer Block
    """
    drop去掉；
    学restormer, ln->attention->ln->ffn
    """
    def __init__(self, dim, num_head, chsa_head_ratio, window_size=8, shift=0, head_dim=None, qkv_bias=True, mv_ver=1,
                 hidden_ratio=2.0, act_layer=nn.GELU, norm_layer=ReshapeLayerNorm, attn_drop=0.0, proj_drop=0.0, drop_path=0.0, helper=True,
                 mv_act=nn.LeakyReLU, exp_factor=1.2, expand_groups=4):
...
if __name__ == '__main__':
    input = torch.randn(1, 32, 288, 384)
    ramit = DRAMiTransformer(dim=32, num_head=8, chsa_head_ratio=0.5)
    out, sp, ch, attn0 = ramit(input)
    print(out.shape)
